...
void llist_rand_push(llist *list, void *data)
{
    struct node *head;
    struct node *new_node;
    if (list == NULL || *list == NULL) {
    ...
    for(int i = 0; i < rand(); i++){
        //walk through list random steps
        new_node->data = data;
        new_node->next = head;
...
void *llist_rand_del(llist *list)
{
    void *popped_data;
    struct node *head = *list;
    ...
    popped_data = head->data;
    struct node *aux = *popped_data;

    for(int i = 0; i < rand(); i++){
        /*walks random steps in list*/
        aux = popped_data;
        popped_data->popped_data->next;
    }
